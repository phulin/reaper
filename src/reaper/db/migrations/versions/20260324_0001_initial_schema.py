...
def upgrade() -> None:
    ...
    op.create_index("ix_functions_original_symbol_name", "functions", ["original_symbol_name"])
    op.create_index("ix_functions_proposed_name", "functions", ["proposed_name"])
    op.create_index("ix_functions_tags_gin", "functions", ["tags"], postgresql_using="gin")
    op.execute(
    ...
    op.create_table(
        ...
        sa.Column("supersedes_id", sa.Integer(), sa.ForeignKey("annotations.id", ondelete="SET NULL")),
        sa.Column("body", sa.Text(), nullable=False),
        sa.Column("payload", postgresql.JSONB(astext_type=sa.Text())),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.CheckConstraint("confidence >= 0.0 AND confidence <= 1.0", name="ck_annotations_confidence"),
        sa.CheckConstraint("version >= 1", name="ck_annotations_version"),
    )
    op.create_index("ix_annotations_agent_target", "annotations", ["agent_id", "target_type", "target_id"])
    op.create_index("ix_annotations_target", "annotations", ["target_type", "target_id"])

    op.create_table(
        "call_graph_edges",
        sa.Column("id", sa.Integer(), primary_key=True),
        sa.Column("caller_function_id", sa.Integer(), sa.ForeignKey("functions.id", ondelete="CASCADE"), nullable=False),
        sa.Column("callee_function_id", sa.Integer(), sa.ForeignKey("functions.id", ondelete="CASCADE"), nullable=False),
    ...
    op.create_index("ix_call_graph_edges_callee", "call_graph_edges", ["callee_function_id"])
    op.create_index("ix_call_graph_edges_caller", "call_graph_edges", ["caller_function_id"])
    op.create_index(
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.CheckConstraint("kind IN ('struct', 'enum', 'typedef')", name="ck_data_types_kind"),
    )
